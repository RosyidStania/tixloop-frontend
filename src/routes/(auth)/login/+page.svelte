<script>
  ...
  async function handleLogin() {
    ...
    try {
      ...
      window.location.href = '/home'; // Pindah ke Home
    } catch (error) {
      console.error(error);
...
<main class="bg-[#0A0910] min-h-screen text-white font-sans flex flex-col">
  ...
  <div class="flex-1 px-4 pt-8 pb-10 space-y-5">
    ...
    <div class="flex items-center gap-3">
      ...
      <span class="text-[11px] text-gray-600">atau masuk dengan email</span>
      <div class="flex-1 h-px bg-[#232033]"></div>
    </div>
    ...
    <div class="space-y-3">
      <div>
        ...
        <input bind:value={email} type="email" placeholder="contoh@email.com" class="w-full bg-[#14121E] border border-[#232033] rounded-xl px-4 py-3 text-sm text-white placeholder-gray-600 outline-none focus:border-[#AAEF45]/60 transition-colors" />
      </div>

      <div>
        <div class="flex items-center justify-between mb-1.5">
          <label class="text-[11px] font-semibold text-gray-400">Password</label>
          <a href="/forgot-password" class="text-[11px] text-[#AAEF45] font-semibold">Lupa password?</a>
        </div>
        <div class="relative">
          <input bind:value={password} type={showPassword ? 'text' : 'password'} placeholder="Masukkan password" class="w-full bg-[#14121E] border border-[#232033] rounded-xl px-4 py-3 pr-11 text-sm text-white placeholder-gray-600 outline-none focus:border-[#AAEF45]/60 transition-colors" />
...
<style>
  input:-webkit-autofill {
    -webkit-box-shadow: 0 0 0 1000px #14121E inset !important;
