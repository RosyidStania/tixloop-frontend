<script>
  ...
  let filteredListings = $derived(
    listings.filter(item => {
      const matchCategory = activeCategory === 'All' || item.ticket?.event?.category?.toLowerCase() === activeCategory.toLowerCase();
...
<main class="bg-[#0A0910] min-h-screen text-white font-sans pb-24 overflow-x-hidden pt-12">
  <div class="px-4">
    ...
    <div class="relative mb-6">
      ...
      <input type="text" bind:value={searchQuery} placeholder="Search event, artist, city..." class="w-full bg-[#1A1825] text-sm text-white rounded-2xl py-3.5 pl-12 pr-4 outline-none focus:ring-1 focus:ring-[#D4FF00] placeholder-gray-500" />
    </div>
