<script>
  ...
  let filteredListings = $derived(
    listings.filter(item => {
      ...
      const matchPrice = item.current_asking_price >= filterCriteria.minPrice && item.current_asking_price <= filterCriteria.maxPrice;
  ...
  onMount(async () => {
    try {
      ...
      let allListings = response.data.data || response.data || [];
      
      // Menerapkan logika sinkronisasi frontend (karena backend tidak diubah)
      // Sembunyikan listing milik seller ini yang belum di-"Jual" di tab Koleksi
      // Sembunyikan juga yang ditolak lokal oleh admin
      ...
      listings = allListings.filter(item => {
        // Jika tiket adalah milik seller yang login di browser ini
        if (sellerListingIds.includes(String(item.id))) {
           // Hanya tampilkan jika sudah di-publish
           return publishedIds.includes(String(item.id));
        }
        
        // Jika milik orang lain, tampilkan apa adanya dari backend
        return true;
      });
...
<div class="sticky top-0 z-40 bg-[#0A0910] px-4 pt-12 pb-2">
  <div class="flex justify-between items-center mb-6 h-8">
    <h1 class="text-2xl font-black text-white tracking-tight leading-none">Jelajahi</h1>
    ...
    <div class="flex overflow-x-auto hide-scrollbar gap-3 mb-4">
      ...
      <div class="space-y-4">
        {#each filteredListings as item}
          <a href="/ticket/{item.id}" class="bg-[#14121E] rounded-2xl p-4 flex gap-4 border border-[#232033] relative block transition-all active:scale-[0.98] hover:border-[#3E3166]">
            ...
            <img src={resolveImageUrl(item.event_poster_url || item.ticket?.event?.event_poster_url || "https://picsum.photos/seed/picsum/200/200")} class="w-24 h-24 rounded-xl object-cover" alt={item.ticket?.event?.name || 'Concert'} />

            
              <div class="flex-1 min-w-0 py-1">
                <div class="flex items-center gap-1.5 mb-2">
                  <span class="text-[10px] text-gray-400 font-bold truncate max-w-[80px]">🎵 {item.ticket?.event?.category || 'Event'}</span>
                  ...
                  <svg class="w-4 h-4 text-[#C7A4FF] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg>
                </div>
                
                <h3 class="font-bold text-sm text-white mb-1 truncate">{item.ticket?.event?.name || 'Event Tidak Diketahui'}</h3>
                ...
                <div class="flex justify-between items-center">
                  <p class="text-[#D4FF00] font-bold text-base tracking-wide truncate pr-2">{formatCurrency(item.current_asking_price)}</p>
