...
<div class="px-4">
  <div class="flex justify-between items-center mb-4">
    ...
    <a href="/explore" class="text-[#D4FF00] text-xs font-semibold flex items-center gap-1">Lihat Semua <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg></a>
  </div>
  ...
  <div class="space-y-3">
    {#each listings.slice(0, 3) as item}
      <a href={`/ticket/${item.id}`} class="block bg-[#14121E] rounded-2xl p-3 flex gap-4 items-center border border-[#232033] hover:border-[#3a3850] transition-colors">
        <img src={item.ticket?.event?.event_poster_url || item.ticket?.event?.poster_url || "https://images.unsplash.com/photo-1459749411175-04bf5292ceea?w=200&h=200&fit=crop"} class="w-20 h-20 rounded-xl object-cover" alt={item.ticket?.event?.name || 'Concert'} />
        <div class="p-4">
